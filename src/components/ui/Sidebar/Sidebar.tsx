...
const Sidebar = ({ userData }: { userData: UserDataType }) => {
  ...
  const [totalNotificationsUnread, setTotalNotificationsUnread] = React.useState(null);
    ...
    <>
      <div
        className={` flex flex-col justify-between fixed h-full overflow-hidden  transition-all duration-200 ${
          showSearchUser || showNotifications ? 'w-[0%]' : 'w-[14%]  border-r border-gray-600'
        }`}
      >
        <div className=" py-4 px-3">
          <Link href="/">
            <h2 className={` py-7 px-3 text-3xl  ${dancingScript.className}`}>Instagram</h2>
          </Link>
          <ul className="flex w-full flex-col items-start  gap-4">
            <Link href={'/'} className="w-full">
              <SidebarItem icon={<HomeIcon />} name="Home" />
            </Link>
            <SidebarItem icon={<NewPostIcon />} name="Create" onClick={() => openModal()} />
            <SidebarItem
              icon={<HeartIcon />}
              name="Notifications"
      ...
      <Notifications
        ...
        setTotalNotificationsUnread={
          setTotalNotificationsUnread as React.Dispatch<React.SetStateAction<number | null>>
        }
      />
    </>
  );
