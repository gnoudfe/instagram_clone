...
const ProfileInfor = ({ userData, isCurrentUser, currentUserId }: ProfileInforProps) => {
  ...
  const isReceiveFriendRequest = userData?.sentFriendRequests?.includes(currentUserId);
  ...
  const handleSendFriendRequest = async () => {
    try {
      ...
      const response = await sendFriendRequestMutation.mutateAsync(userData?._id);
  ...
  const handleAcceptFriendRequest = async () => {
    try {
      setIsLoading(true);
      const response = await acceptFriendRequestMutation.mutateAsync(userData?._id);
      if (response.status === 'success') {
        router.refresh();
      }
      console.log(response);
  ...
  const handleRejectFriendRequest = async () => {
    try {
      setIsLoading(true);
      const response = await rejectFriendRequestMutation.mutateAsync(userData?._id);
      if (response.status === 'success') {
        router.refresh();
      }
      console.log(response);
  ...
  return (
    <>
      <div className="flex gap-[60px] w-full max-w-[935px]  pt-[30px]  items-center">
        ...
        <div className="flex flex-col gap-6 pt-[10px]">
          <div className="flex items-center gap-6">
            ...
            {isCurrentUser ? (
              <CurrentUserSettings handleShowModalSettings={handleShowModalSettings} />
            ) : isSentFriendRequest ? (
              <SendFriendRequest />
          ...
          <div className="flex items-center gap-4">
            <span className="text-base font-normal text-[#A8A8A8]">
              <span className="text-white font-semibold">{userData?.totalPosts}</span> posts
            </span>
            <span
              className="text-base font-normal text-[#A8A8A8] cursor-pointer hover:opacity-80 transition-all duration-300"
              onClick={handleShowModalFriends}
            >
              <span className="text-white font-semibold">{userData?.totalFriends}</span> followers
            </span>
          </div>
      ...
      {isShowModalFriends && <ModalFriends onClose={handleCloseModalFriends} />}
    </>
  );
};
