...
import { useGetOtherUserPostsQuery, useGetPostDetail } from '@/services/queries/usePost';
...
const PostProfileLayout = ({ id, slug }: { id: string; slug: string }) => {
  ...
  const { data: listPosts, isLoading: isLoadingMorePosts } = useGetOtherUserPostsQuery({
    userId: slug,
  });
      ...
      <div className="mt-4 w-[100%] flex flex-col  gap-12 items-center justify-center  pl-[310px]">
        ...
        <div className="w-full max-w-[900px]">
          ...
          <span className="text-sm font-normal block mt-4">
            More posts from{' '}
            <span className="text-white font-semibold">{data?.post?.user?.username}</span>
          </span>
